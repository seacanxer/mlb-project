...
def insert_bet(bet):
    """Keep one lock per (fixture, market). If an unsettled lock already exists
    for the same match+start_ts+market, keep the higher EV. If it is already
    settled, skip entirely so scans never stack duplicates."""
    conn = _connect()
    c = conn.cursor()
    existing = c.execute('''
        SELECT id, ev, settled FROM bets
        WHERE match=? AND start_ts=? AND market=? AND start_ts IS NOT NULL
        ORDER BY settled DESC, ev DESC
        LIMIT 1
    ''', (
        bet.get('match'), bet.get('start_ts'), bet.get('market')
    )).fetchone()
    if existing:
        if existing['settled']:
            conn.close()
