...
def main():
    ...
    for m in sorted(future, key=lambda x: float(x['info']['start_ts'])):
        ...
        try:
            ...
            quote_path = os.path.join(FC_DIR, 'odds_quotes.jsonl')
            with open(quote_path, 'a', encoding='utf-8') as quote_file:
                quote = {
                ...
                quote['secondary'] = secondary
                quote_file.write(json.dumps(quote, ensure_ascii=False, allow_nan=False) + '\n')
            time.sleep(0.25)
        except Exception:
